...
struct VideoPlayerView: View {
      ...
      VStack {
        ...
        HStack {
          ...
          Button(action: {
            ...
            CustomIcon(isFullScreen ? "arrow.down.forward.and.arrow.up.backward" : "arrow.up.left.and.arrow.down.right", color: UIControlsProps?.fullScreen.color)
        ...
        Menu(option.key) {
          ForEach(values!, id: \.self) { item in
            Button(action: {
              ...
              let selectedName = item["name"] as Any
              
              controls.optionSelected(option.key, value)
              initialSelectedOption = selectedName as! String
            }) {
              if let name = item["name"] as? String {
                if initialSelectedOption != "" {
                  if name == initialSelectedOption {
  ...
  func VideoSeekerThumbnailView(geometry: GeometryProxy) -> some View {
    let calculatedWidthThumbnailSizeByWidth = calculateSizeByWidth(150, 0.4)
